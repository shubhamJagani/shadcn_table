...
const TableFooter = React.forwardRef(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn(
      "border-t border-neutral-200 bg-neutral-100/50 font-medium [&>tr]:last:border-b-0 dark:border-neutral-800 dark:bg-neutral-800/50",
      className
    )}
    {...props} />
));
...
const TableHead = React.forwardRef(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      "h-12 px-3 text-left align-middle font-medium text-neutral-500 [&:has([role=checkbox])]:w-px [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-0.5 dark:text-neutral-400",
      className
    )}
    {...props} />
));
